...
/**
 * The base condition-bean of impasse_keywords.
 * @author DBFlute(AutoGenerator)
 */
public class BsImpasseKeywordsCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected ImpasseKeywordsCQ _conditionQuery;

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public BsImpasseKeywordsCB() {
    ...
    public void acceptPrimaryKey(final Integer id) {
        ...
        final BsImpasseKeywordsCB cb = this;
     ...
     */
    public ImpasseKeywordsCQ query() {
        assertQueryPurpose(); // assert only when user-public query 
        return getConditionQuery();
    }

    public ImpasseKeywordsCQ getConditionQuery() { // public for parameter comment and internal
    ...
    protected ImpasseKeywordsCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause()
                .getBasePointAliasName(), 0);
    }

    protected ImpasseKeywordsCQ xcreateCQ(final ConditionQuery childQuery,
            final SqlClause sqlClause, final String aliasName,
            final int nestLevel) {
        final ImpasseKeywordsCQ cq = xnewCQ(childQuery, sqlClause, aliasName,
                nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected ImpasseKeywordsCQ xnewCQ(final ConditionQuery childQuery,
            final SqlClause sqlClause, final String aliasName,
            final int nestLevel) {
        return new ImpasseKeywordsCQ(childQuery, sqlClause, aliasName,
                nestLevel);
    }
     ...
     * <pre>
     * cb.query().<span style="color: #FD4747">union</span>(new UnionQuery&lt;ImpasseKeywordsCB&gt;() {
     *     public void query(ImpasseKeywordsCB unionCB) {
     ...
     */
    public void union(final UnionQuery<ImpasseKeywordsCB> unionQuery) {
        final ImpasseKeywordsCB cb = new ImpasseKeywordsCB();
        cb.xsetupForUnion(this);
        xsyncUQ(cb);
        unionQuery.query(cb);
        xsaveUCB(cb);
        final ImpasseKeywordsCQ cq = cb.query();
     ...
     * because it inherits calls before. (Don't call SetupSelect after here)
     * <pre>
     * cb.query().<span style="color: #FD4747">unionAll</span>(new UnionQuery&lt;ImpasseKeywordsCB&gt;() {
     *     public void query(ImpasseKeywordsCB unionCB) {
     *         unionCB.query().setXxx...
     *     }
     * });
     ...
     */
    public void unionAll(final UnionQuery<ImpasseKeywordsCB> unionQuery) {
        final ImpasseKeywordsCB cb = new ImpasseKeywordsCB();
        cb.xsetupForUnion(this);
        xsyncUQ(cb);
        unionQuery.query(cb);
    ...
    public HpSpecification specify() {
        ...
        if (_specification == null) {
            _specification = new HpSpecification(this,
                    new HpSpQyCall<ImpasseKeywordsCQ>() {
                        @Override
                        public boolean has() {
                            return true;
                        }
    ...
    public static class HpSpecification extends
            HpAbstractSpecification<ImpasseKeywordsCQ> {
        public HpSpecification(final ConditionBean baseCB,
                final HpSpQyCall<ImpasseKeywordsCQ> qyCall,
         ...
         * @return The information object of specified column. (NotNull)
         */
        public HpSpecifiedColumn columnKeyword() {
            return doColumn("keyword");
        }
         ...
         */
        public HpSDRFunction<ImpasseKeywordsCB, ImpasseKeywordsCQ> myselfDerived() {
            assertDerived("${tmpPropertyName}");
            if (xhasSyncQyCall()) {
                xsyncQyCall().qy();
            } // for sync (for example, this in ColumnQuery)
            return new HpSDRFunction<ImpasseKeywordsCB, ImpasseKeywordsCQ>(
                    _baseCB, _qyCall.qy(),
                    new HpSDRSetupper<ImpasseKeywordsCB, ImpasseKeywordsCQ>() {
                        @Override
                        public void setup(final String function,
                                final SubQuery<ImpasseKeywordsCB> subQuery,
                                final ImpasseKeywordsCQ cq,
     ...
     * cb.<span style="color: #FD4747">columnQuery</span>(new SpecifyQuery&lt;ImpasseKeywordsCB&gt;() {
     *     public void query(ImpasseKeywordsCB cb) {
     *         cb.specify().<span style="color: #FD4747">columnFoo()</span>; <span style="color: #3F7E5E">// left column</span>
     *     }
     * }).lessThan(new SpecifyQuery&lt;ImpasseKeywordsCB&gt;() {
     *     public void query(ImpasseKeywordsCB cb) {
     ...
     */
    public HpColQyOperand<ImpasseKeywordsCB> columnQuery(
            final SpecifyQuery<ImpasseKeywordsCB> leftSpecifyQuery) {
        return new HpColQyOperand<ImpasseKeywordsCB>(
                new HpColQyHandler<ImpasseKeywordsCB>() {
                    @Override
                    public HpCalculator handle(
                            final SpecifyQuery<ImpasseKeywordsCB> rightSp,
    ...
    protected ImpasseKeywordsCB xcreateColumnQueryCB() {
        final ImpasseKeywordsCB cb = new ImpasseKeywordsCB();
     ...
     */
    public ImpasseKeywordsCB dreamCruiseCB() {
        final ImpasseKeywordsCB cb = new ImpasseKeywordsCB();
     ...
     * cb.<span style="color: #FD4747">orScopeQuery</span>(new OrQuery&lt;ImpasseKeywordsCB&gt;() {
     *     public void query(ImpasseKeywordsCB orCB) {
     ...
     */
    public void orScopeQuery(final OrQuery<ImpasseKeywordsCB> orQuery) {
        xorSQ((ImpasseKeywordsCB) this, orQuery);
    }
     ...
     *     public void query(ImpasseKeywordsCB orCB) {
     *         orCB.query().setFOO_Equal...
     *         orCB.<span style="color: #FD4747">orScopeQueryAndPart</span>(new AndQuery&lt;ImpasseKeywordsCB&gt;() {
     *             public void query(ImpasseKeywordsCB andCB) {
     *                 andCB.query().setBar_...
     *                 andCB.query().setQux_...
     *             }
     *         });
     ...
     */
    public void orScopeQueryAndPart(final AndQuery<ImpasseKeywordsCB> andQuery) {
        xorSQAP((ImpasseKeywordsCB) this, andQuery);
    }
    ...
    protected void xprepareSyncQyCall(final ConditionBean mainCB) {
        ...
        if (mainCB != null) {
            cb = (ImpasseKeywordsCB) mainCB;
        } else {
            cb = new ImpasseKeywordsCB();
        }
        specify().xsetSyncQyCall(new HpSpQyCall<ImpasseKeywordsCQ>() {
            @Override
            public boolean has() {
                return true;
            }
