...
/**
 * The base condition-bean of impasse_nodes.
 * @author DBFlute(AutoGenerator)
 */
public class BsImpasseNodesCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected ImpasseNodesCQ _conditionQuery;

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public BsImpasseNodesCB() {
    ...
    public void acceptPrimaryKey(final Integer id) {
        ...
        final BsImpasseNodesCB cb = this;
     ...
     */
    public ImpasseNodesCQ query() {
        assertQueryPurpose(); // assert only when user-public query 
        return getConditionQuery();
    }

    public ImpasseNodesCQ getConditionQuery() { // public for parameter comment and internal
    ...
    protected ImpasseNodesCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause()
                .getBasePointAliasName(), 0);
    }

    protected ImpasseNodesCQ xcreateCQ(final ConditionQuery childQuery,
            final SqlClause sqlClause, final String aliasName,
            final int nestLevel) {
        final ImpasseNodesCQ cq = xnewCQ(childQuery, sqlClause, aliasName,
                nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected ImpasseNodesCQ xnewCQ(final ConditionQuery childQuery,
            final SqlClause sqlClause, final String aliasName,
            final int nestLevel) {
        return new ImpasseNodesCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     * <pre>
     * cb.query().<span style="color: #FD4747">union</span>(new UnionQuery&lt;ImpasseNodesCB&gt;() {
     *     public void query(ImpasseNodesCB unionCB) {
     ...
     */
    public void union(final UnionQuery<ImpasseNodesCB> unionQuery) {
        final ImpasseNodesCB cb = new ImpasseNodesCB();
        cb.xsetupForUnion(this);
        xsyncUQ(cb);
        unionQuery.query(cb);
        xsaveUCB(cb);
        final ImpasseNodesCQ cq = cb.query();
     ...
     * because it inherits calls before. (Don't call SetupSelect after here)
     * <pre>
     * cb.query().<span style="color: #FD4747">unionAll</span>(new UnionQuery&lt;ImpasseNodesCB&gt;() {
     *     public void query(ImpasseNodesCB unionCB) {
     *         unionCB.query().setXxx...
     *     }
     * });
     ...
     */
    public void unionAll(final UnionQuery<ImpasseNodesCB> unionQuery) {
        final ImpasseNodesCB cb = new ImpasseNodesCB();
        cb.xsetupForUnion(this);
        xsyncUQ(cb);
        unionQuery.query(cb);
    ...
    public static class HpSpecification extends
            HpAbstractSpecification<ImpasseNodesCQ> {
        public HpSpecification(final ConditionBean baseCB,
                final HpSpQyCall<ImpasseNodesCQ> qyCall,
         ...
         * @return The information object of specified column. (NotNull)
         */
        public HpSpecifiedColumn columnNodeTypeId() {
            return doColumn("node_type_id");
        }
         ...
         * @return The information object of specified column. (NotNull)
         */
        public HpSpecifiedColumn columnParentId() {
            return doColumn("parent_id");
        }
         ...
         * @return The information object of specified column. (NotNull)
         */
        public HpSpecifiedColumn columnPath() {
            return doColumn("path");
        }
         ...
         */
        public HpSDRFunction<ImpasseNodesCB, ImpasseNodesCQ> myselfDerived() {
            assertDerived("${tmpPropertyName}");
            if (xhasSyncQyCall()) {
                xsyncQyCall().qy();
            } // for sync (for example, this in ColumnQuery)
            return new HpSDRFunction<ImpasseNodesCB, ImpasseNodesCQ>(_baseCB,
                    _qyCall.qy(),
                    new HpSDRSetupper<ImpasseNodesCB, ImpasseNodesCQ>() {
                        @Override
                        public void setup(final String function,
                                final SubQuery<ImpasseNodesCB> subQuery,
                                final ImpasseNodesCQ cq,
     ...
     * cb.<span style="color: #FD4747">columnQuery</span>(new SpecifyQuery&lt;ImpasseNodesCB&gt;() {
     *     public void query(ImpasseNodesCB cb) {
     *         cb.specify().<span style="color: #FD4747">columnFoo()</span>; <span style="color: #3F7E5E">// left column</span>
     *     }
     * }).lessThan(new SpecifyQuery&lt;ImpasseNodesCB&gt;() {
     *     public void query(ImpasseNodesCB cb) {
     ...
     */
    public HpColQyOperand<ImpasseNodesCB> columnQuery(
            final SpecifyQuery<ImpasseNodesCB> leftSpecifyQuery) {
        return new HpColQyOperand<ImpasseNodesCB>(
                new HpColQyHandler<ImpasseNodesCB>() {
                    @Override
                    public HpCalculator handle(
                            final SpecifyQuery<ImpasseNodesCB> rightSp,
    ...
    protected ImpasseNodesCB xcreateColumnQueryCB() {
        final ImpasseNodesCB cb = new ImpasseNodesCB();
     ...
     */
    public ImpasseNodesCB dreamCruiseCB() {
        final ImpasseNodesCB cb = new ImpasseNodesCB();
     ...
     * cb.<span style="color: #FD4747">orScopeQuery</span>(new OrQuery&lt;ImpasseNodesCB&gt;() {
     *     public void query(ImpasseNodesCB orCB) {
     ...
     */
    public void orScopeQuery(final OrQuery<ImpasseNodesCB> orQuery) {
        xorSQ((ImpasseNodesCB) this, orQuery);
    }
     ...
     *     public void query(ImpasseNodesCB orCB) {
     *         orCB.query().setFOO_Equal...
     *         orCB.<span style="color: #FD4747">orScopeQueryAndPart</span>(new AndQuery&lt;ImpasseNodesCB&gt;() {
     *             public void query(ImpasseNodesCB andCB) {
     *                 andCB.query().setBar_...
     *                 andCB.query().setQux_...
     *             }
     *         });
     ...
     */
    public void orScopeQueryAndPart(final AndQuery<ImpasseNodesCB> andQuery) {
        xorSQAP((ImpasseNodesCB) this, andQuery);
    }
    ...
    protected void xprepareSyncQyCall(final ConditionBean mainCB) {
        ...
        if (mainCB != null) {
            cb = (ImpasseNodesCB) mainCB;
        } else {
            cb = new ImpasseNodesCB();
        }
        specify().xsetSyncQyCall(new HpSpQyCall<ImpasseNodesCQ>() {
            @Override
            public boolean has() {
                return true;
            }
