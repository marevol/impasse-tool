...
/**
 * The base condition-bean of impasse_test_plans.
 * @author DBFlute(AutoGenerator)
 */
public class BsImpasseTestPlansCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected ImpasseTestPlansCQ _conditionQuery;

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public BsImpasseTestPlansCB() {
    ...
    public void acceptPrimaryKey(final Integer id) {
        ...
        final BsImpasseTestPlansCB cb = this;
     ...
     */
    public ImpasseTestPlansCQ query() {
        assertQueryPurpose(); // assert only when user-public query 
        return getConditionQuery();
    }

    public ImpasseTestPlansCQ getConditionQuery() { // public for parameter comment and internal
    ...
    protected ImpasseTestPlansCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause()
                .getBasePointAliasName(), 0);
    }

    protected ImpasseTestPlansCQ xcreateCQ(final ConditionQuery childQuery,
            final SqlClause sqlClause, final String aliasName,
            final int nestLevel) {
        final ImpasseTestPlansCQ cq = xnewCQ(childQuery, sqlClause, aliasName,
                nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected ImpasseTestPlansCQ xnewCQ(final ConditionQuery childQuery,
            final SqlClause sqlClause, final String aliasName,
            final int nestLevel) {
        return new ImpasseTestPlansCQ(childQuery, sqlClause, aliasName,
                nestLevel);
    }
     ...
     * <pre>
     * cb.query().<span style="color: #FD4747">union</span>(new UnionQuery&lt;ImpasseTestPlansCB&gt;() {
     *     public void query(ImpasseTestPlansCB unionCB) {
     ...
     */
    public void union(final UnionQuery<ImpasseTestPlansCB> unionQuery) {
        final ImpasseTestPlansCB cb = new ImpasseTestPlansCB();
        cb.xsetupForUnion(this);
        xsyncUQ(cb);
        unionQuery.query(cb);
        xsaveUCB(cb);
        final ImpasseTestPlansCQ cq = cb.query();
     ...
     * because it inherits calls before. (Don't call SetupSelect after here)
     * <pre>
     * cb.query().<span style="color: #FD4747">unionAll</span>(new UnionQuery&lt;ImpasseTestPlansCB&gt;() {
     *     public void query(ImpasseTestPlansCB unionCB) {
     *         unionCB.query().setXxx...
     *     }
     * });
     ...
     */
    public void unionAll(final UnionQuery<ImpasseTestPlansCB> unionQuery) {
        final ImpasseTestPlansCB cb = new ImpasseTestPlansCB();
        cb.xsetupForUnion(this);
        xsyncUQ(cb);
        unionQuery.query(cb);
    ...
    public HpSpecification specify() {
        ...
        if (_specification == null) {
            _specification = new HpSpecification(this,
                    new HpSpQyCall<ImpasseTestPlansCQ>() {
                        @Override
                        public boolean has() {
                            return true;
                        }
    ...
    public static class HpSpecification extends
            HpAbstractSpecification<ImpasseTestPlansCQ> {
        public HpSpecification(final ConditionBean baseCB,
                final HpSpQyCall<ImpasseTestPlansCQ> qyCall,
         ...
         * @return The information object of specified column. (NotNull)
         */
        public HpSpecifiedColumn columnVersionId() {
            return doColumn("version_id");
        }
         ...
         */
        public HpSDRFunction<ImpasseTestPlansCB, ImpasseTestPlansCQ> myselfDerived() {
            assertDerived("${tmpPropertyName}");
            if (xhasSyncQyCall()) {
                xsyncQyCall().qy();
            } // for sync (for example, this in ColumnQuery)
            return new HpSDRFunction<ImpasseTestPlansCB, ImpasseTestPlansCQ>(
                    _baseCB,
                    _qyCall.qy(),
                    new HpSDRSetupper<ImpasseTestPlansCB, ImpasseTestPlansCQ>() {
                        @Override
                        public void setup(final String function,
                                final SubQuery<ImpasseTestPlansCB> subQuery,
                                final ImpasseTestPlansCQ cq,
     ...
     * cb.<span style="color: #FD4747">columnQuery</span>(new SpecifyQuery&lt;ImpasseTestPlansCB&gt;() {
     *     public void query(ImpasseTestPlansCB cb) {
     *         cb.specify().<span style="color: #FD4747">columnFoo()</span>; <span style="color: #3F7E5E">// left column</span>
     *     }
     * }).lessThan(new SpecifyQuery&lt;ImpasseTestPlansCB&gt;() {
     *     public void query(ImpasseTestPlansCB cb) {
     ...
     */
    public HpColQyOperand<ImpasseTestPlansCB> columnQuery(
            final SpecifyQuery<ImpasseTestPlansCB> leftSpecifyQuery) {
        return new HpColQyOperand<ImpasseTestPlansCB>(
                new HpColQyHandler<ImpasseTestPlansCB>() {
                    @Override
                    public HpCalculator handle(
                            final SpecifyQuery<ImpasseTestPlansCB> rightSp,
    ...
    protected ImpasseTestPlansCB xcreateColumnQueryCB() {
        final ImpasseTestPlansCB cb = new ImpasseTestPlansCB();
     ...
     */
    public ImpasseTestPlansCB dreamCruiseCB() {
        final ImpasseTestPlansCB cb = new ImpasseTestPlansCB();
     ...
     * cb.<span style="color: #FD4747">orScopeQuery</span>(new OrQuery&lt;ImpasseTestPlansCB&gt;() {
     *     public void query(ImpasseTestPlansCB orCB) {
     ...
     */
    public void orScopeQuery(final OrQuery<ImpasseTestPlansCB> orQuery) {
        xorSQ((ImpasseTestPlansCB) this, orQuery);
    }
     ...
     *     public void query(ImpasseTestPlansCB orCB) {
     *         orCB.query().setFOO_Equal...
     *         orCB.<span style="color: #FD4747">orScopeQueryAndPart</span>(new AndQuery&lt;ImpasseTestPlansCB&gt;() {
     *             public void query(ImpasseTestPlansCB andCB) {
     *                 andCB.query().setBar_...
     *                 andCB.query().setQux_...
     *             }
     *         });
     ...
     */
    public void orScopeQueryAndPart(final AndQuery<ImpasseTestPlansCB> andQuery) {
        xorSQAP((ImpasseTestPlansCB) this, andQuery);
    }
    ...
    protected void xprepareSyncQyCall(final ConditionBean mainCB) {
        ...
        if (mainCB != null) {
            cb = (ImpasseTestPlansCB) mainCB;
        } else {
            cb = new ImpasseTestPlansCB();
        }
        specify().xsetSyncQyCall(new HpSpQyCall<ImpasseTestPlansCQ>() {
            @Override
            public boolean has() {
                return true;
            }
