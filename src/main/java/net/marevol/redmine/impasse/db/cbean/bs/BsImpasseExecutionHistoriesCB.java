...
/**
 * The base condition-bean of impasse_execution_histories.
 * @author DBFlute(AutoGenerator)
 */
public class BsImpasseExecutionHistoriesCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected ImpasseExecutionHistoriesCQ _conditionQuery;

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public BsImpasseExecutionHistoriesCB() {
    ...
    public void acceptPrimaryKey(final Integer id) {
        ...
        final BsImpasseExecutionHistoriesCB cb = this;
     ...
     */
    public ImpasseExecutionHistoriesCQ query() {
        assertQueryPurpose(); // assert only when user-public query 
        return getConditionQuery();
    }

    public ImpasseExecutionHistoriesCQ getConditionQuery() { // public for parameter comment and internal
    ...
    protected ImpasseExecutionHistoriesCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause()
                .getBasePointAliasName(), 0);
    }

    protected ImpasseExecutionHistoriesCQ xcreateCQ(
            final ConditionQuery childQuery, final SqlClause sqlClause,
            final String aliasName, final int nestLevel) {
        final ImpasseExecutionHistoriesCQ cq = xnewCQ(childQuery, sqlClause,
                aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected ImpasseExecutionHistoriesCQ xnewCQ(
            final ConditionQuery childQuery, final SqlClause sqlClause,
            final String aliasName, final int nestLevel) {
        return new ImpasseExecutionHistoriesCQ(childQuery, sqlClause,
                aliasName, nestLevel);
    }
     ...
     * <pre>
     * cb.query().<span style="color: #FD4747">union</span>(new UnionQuery&lt;ImpasseExecutionHistoriesCB&gt;() {
     *     public void query(ImpasseExecutionHistoriesCB unionCB) {
     ...
     */
    public void union(final UnionQuery<ImpasseExecutionHistoriesCB> unionQuery) {
        final ImpasseExecutionHistoriesCB cb = new ImpasseExecutionHistoriesCB();
        cb.xsetupForUnion(this);
        xsyncUQ(cb);
        unionQuery.query(cb);
        xsaveUCB(cb);
        final ImpasseExecutionHistoriesCQ cq = cb.query();
     ...
     * because it inherits calls before. (Don't call SetupSelect after here)
     * <pre>
     * cb.query().<span style="color: #FD4747">unionAll</span>(new UnionQuery&lt;ImpasseExecutionHistoriesCB&gt;() {
     *     public void query(ImpasseExecutionHistoriesCB unionCB) {
     *         unionCB.query().setXxx...
     *     }
     * });
    ...
    public void unionAll(
            final UnionQuery<ImpasseExecutionHistoriesCB> unionQuery) {
        final ImpasseExecutionHistoriesCB cb = new ImpasseExecutionHistoriesCB();
        cb.xsetupForUnion(this);
        xsyncUQ(cb);
        unionQuery.query(cb);
    ...
    public HpSpecification specify() {
        ...
        if (_specification == null) {
            _specification = new HpSpecification(this,
                    new HpSpQyCall<ImpasseExecutionHistoriesCQ>() {
                        @Override
                        public boolean has() {
                            return true;
                        }
                        ...
                        @Override
                        public ImpasseExecutionHistoriesCQ qy() {
                            return getConditionQuery();
                        }
    ...
    public static class HpSpecification extends
            HpAbstractSpecification<ImpasseExecutionHistoriesCQ> {
        public HpSpecification(final ConditionBean baseCB,
                final HpSpQyCall<ImpasseExecutionHistoriesCQ> qyCall,
         ...
         */
        public HpSDRFunction<ImpasseExecutionHistoriesCB, ImpasseExecutionHistoriesCQ> myselfDerived() {
            assertDerived("${tmpPropertyName}");
            if (xhasSyncQyCall()) {
                xsyncQyCall().qy();
            } // for sync (for example, this in ColumnQuery)
            return new HpSDRFunction<ImpasseExecutionHistoriesCB, ImpasseExecutionHistoriesCQ>(
                    _baseCB,
                    _qyCall.qy(),
                    new HpSDRSetupper<ImpasseExecutionHistoriesCB, ImpasseExecutionHistoriesCQ>() {
                        @Override
                        public void setup(
                                final String function,
                                final SubQuery<ImpasseExecutionHistoriesCB> subQuery,
                                final ImpasseExecutionHistoriesCQ cq,
     ...
     * cb.<span style="color: #FD4747">columnQuery</span>(new SpecifyQuery&lt;ImpasseExecutionHistoriesCB&gt;() {
     *     public void query(ImpasseExecutionHistoriesCB cb) {
     *         cb.specify().<span style="color: #FD4747">columnFoo()</span>; <span style="color: #3F7E5E">// left column</span>
     *     }
     * }).lessThan(new SpecifyQuery&lt;ImpasseExecutionHistoriesCB&gt;() {
     *     public void query(ImpasseExecutionHistoriesCB cb) {
     ...
     */
    public HpColQyOperand<ImpasseExecutionHistoriesCB> columnQuery(
            final SpecifyQuery<ImpasseExecutionHistoriesCB> leftSpecifyQuery) {
        return new HpColQyOperand<ImpasseExecutionHistoriesCB>(
                new HpColQyHandler<ImpasseExecutionHistoriesCB>() {
                    @Override
                    public HpCalculator handle(
                            final SpecifyQuery<ImpasseExecutionHistoriesCB> rightSp,
    ...
    protected ImpasseExecutionHistoriesCB xcreateColumnQueryCB() {
        final ImpasseExecutionHistoriesCB cb = new ImpasseExecutionHistoriesCB();
     ...
     */
    public ImpasseExecutionHistoriesCB dreamCruiseCB() {
        final ImpasseExecutionHistoriesCB cb = new ImpasseExecutionHistoriesCB();
     ...
     * cb.<span style="color: #FD4747">orScopeQuery</span>(new OrQuery&lt;ImpasseExecutionHistoriesCB&gt;() {
     *     public void query(ImpasseExecutionHistoriesCB orCB) {
     ...
     */
    public void orScopeQuery(final OrQuery<ImpasseExecutionHistoriesCB> orQuery) {
        xorSQ((ImpasseExecutionHistoriesCB) this, orQuery);
    }
     ...
     *     public void query(ImpasseExecutionHistoriesCB orCB) {
     *         orCB.query().setFOO_Equal...
     *         orCB.<span style="color: #FD4747">orScopeQueryAndPart</span>(new AndQuery&lt;ImpasseExecutionHistoriesCB&gt;() {
     *             public void query(ImpasseExecutionHistoriesCB andCB) {
     *                 andCB.query().setBar_...
     *                 andCB.query().setQux_...
     *             }
     *         });
    ...
    public void orScopeQueryAndPart(
            final AndQuery<ImpasseExecutionHistoriesCB> andQuery) {
        xorSQAP((ImpasseExecutionHistoriesCB) this, andQuery);
    }
    ...
    protected void xprepareSyncQyCall(final ConditionBean mainCB) {
        ...
        if (mainCB != null) {
            cb = (ImpasseExecutionHistoriesCB) mainCB;
        } else {
            cb = new ImpasseExecutionHistoriesCB();
        }
        specify().xsetSyncQyCall(new HpSpQyCall<ImpasseExecutionHistoriesCQ>() {
            @Override
            public boolean has() {
                return true;
            }
